...
- [ ] **GUI Tauri (desktop + Android natif)** : une seule app, une seule UI (React/Svelte) partagée entre desktop et mobile, tunnel natif intégré sur Android.
  - **Desktop** : Tauri 2 (ligne 2.11) + le binaire Go actuel en sidecar (`externalBin`) — list, probe, connect, status et daemon réutilisés tels quels, pont JSON via commandes Tauri.
