...
- [ ] **GUI Tauri (desktop + Android natif)** : une seule app, une seule UI (React) partagée entre desktop et mobile, tunnel natif intégré sur Android.
  - **Desktop (en cours)** : Tauri 2 + le binaire Go en sidecar (`externalBin`). Le backend HTTP est fait : `vpngate serve` expose l'API (`GET /api/health`, `GET /api/servers` avec filtres `country/proto/transport/source/min_score/max_ping/refresh`, `GET /api/status`, `POST /api/connect`, `POST /api/disconnect`, `GET /api/logs`) avec CORS `*`, par défaut sur `127.0.0.1:1865` (`--addr 0.0.0.0:1865` pour un daemon distant). Le frontend React de `gui/` parle HTTP pur (pas d'invoke Tauri), il servira tel quel sur mobile avec une base URL configurable. La fermeture de l'app coupe le stdin du sidecar → arrêt propre du tunnel.
